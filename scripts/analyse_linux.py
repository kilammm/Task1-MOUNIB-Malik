    ...
    try:
        with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                ...
                if match:
                    timestamp, hostname, source, event = match.groups()
                    ...
                    if "error" in event_lower or "failed" in event_lower:
                        ...
                        level = "WARNING"
                    elif "critical" in event_lower or "crit" in event_lower:
                        level = "CRITICAL"
                    else:
                        level = "INFO"

                    log_type = "Security" if "auth" in filepath else "System"
        ...
        print(f"✅ {len(records)} events extracted from {filepath}")

    except PermissionError:
        print(f"❌ Permission denied for {filepath} — run with sudo")
...
def generate_charts(df):
    ...
    plt.title("Error Frequency by Source (System / Security)")
    plt.xlabel("Source")
...
def run_analysis():
    ...
    all_records = []
    all_records += extract_logs(SYSLOG_PATH)
    all_records += extract_logs(AUTHLOG_PATH)

    if not all_records:
        print("❌ No logs extracted — check permissions")
        return

    df = process_logs(all_records)
    print(f"✅ {len(df)} lines processed")
    generate_charts(df)
